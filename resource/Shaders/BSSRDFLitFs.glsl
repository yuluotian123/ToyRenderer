...
//计算点光源光照结果
vec3 calSinglePointLight(uint lightIndex,vec3 WorldPos,vec3 N,vec3 V,vec3 albedo,float rough,float metal) {
...
vec3 calPointLight(vec3 WorldPos,vec3 N,vec3 V,vec3 albedo,float rough,float metal){
    ...
    for(uint i = 0; i < lightCount; i++){
         ...
         color += calSinglePointLight(lightIndex,WorldPos,N,V,albedo,rough,metal);
    }
    return color;
}
//IBL 暂时有bug
...
void main()
{
     ...
     color += calPointLight(f_in.WorldPos,N,V,albedo,rough,metal);
